...
import { AuthShell, Field, Input } from "@/components/auth-shell";
...
function Login() {
  const dashboardHref = `${(import.meta.env.BASE_URL || "/").replace(/\/$/, "")}/app/dashboard`;
  ...
  return (
    <AuthShell title="Sign in to your account" subtitle="Welcome back. Continue your application or manage your license.">
      <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
        ...
        <a
          href={dashboardHref}
          className="inline-flex w-full items-center justify-center rounded-md bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground shadow-elegant transition-colors hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-ring/40"
        >
          Sign in
        </a>
